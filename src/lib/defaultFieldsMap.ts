...
import { createFieldFromDefinition } from "../utils";
...
class DefaultFieldJSON extends Field<FieldTypes.JSON> {
  serialize(value: any, format: SerializerFormat, from: Model): any {
    const _format = (obj: object) => {
      const formattedEntries = Object.entries(this.options.fields ?? {}).map(
        ([slug, def]) => {
          const field = createFieldFromDefinition(def, from.model);
